...
/**
 * Данный класс представляет собой мягкое исключение и применяется в некритических ситуациях, не требующих немедленного
 * реагирования. Для этого он расширяет класс исключения во время выполнения, а также объявляет и реализует наиболее
 * полезные конструкторы и статические поля.
 *
 * @apiNote После возникновения и обработки мягкого исключения обычно можно продолжить работу. Например, когда
 * происходит передача некорректного параметра.
 * @since 3.0.0-RC1
 */
public class SoftException extends RuntimeException {

    /**
     * Данное неизменяемое статическое поле представляет собой нулевой объект класса строки и содержит сообщение мягкого
     ...
     * мягкого исключения по умолчанию.
     *
     * @since 4.0.0-RC3
     */
    ...
    /**
     * Данный конструктор создаёт мягкое исключение на основе {@linkplain #DEFAULT_MESSAGE сообщения},
    ...
    /**
     * Данный конструктор создаёт мягкое исключение на основе переданного сообщения, а также
     * {@linkplain #DEFAULT_CAUSE причины}, {@linkplain #DEFAULT_SUPPRESSION параметров подавления} и
     * {@linkplain #DEFAULT_WRITABLE трассировки стека по умолчанию}.
     *
    ...
    /**
     * Данный конструктор создаёт мягкое исключение на основе переданной причины, а также
    ...
    /**
     * Данный конструктор создаёт мягкое исключение на основе переданного сообщения и причины, а также
    ...
    /**
     * Данный конструктор создаёт мягкое исключение на основе переданных параметров подавления и трассировки стека, а
    ...
    /**
     * Данный конструктор создаёт мягкое исключение на основе переданного сообщения, параметров подавления и трассировки
    ...
    /**
     * Данный конструктор создаёт мягкое исключение на основе переданной причины, параметров подавления и трассировки
    ...
    /**
     * Данный конструктор создаёт мягкое исключение на основе переданного сообщения, причины, параметров подавления и
     * трассировки стека.
     *
     * @param message сообщение.
     * @param cause причина.
