...
public interface Condition {
     ...
     * переданной логической функции шаблонным сообщением), в противном случае возвращает её.
     *
    ...
    /**
     * Вычисляет эту логическую функцию, инициализирует и возвращает её результат вычисления.
     *
     ...
     * метод вычисления которой вызывает метод вычисления этой логической функции и, если её результат вычисления
     * истинный, вызывает метод вычисления переданной в этот метод логической функции, а после возвращает её результат
     ...
     * нулевого объекта (переданной логической функции) с переопределённым сообщением (отформатированным именем
     * переданной логической функции шаблонным сообщением), в противном случае инициализирует другую логическую функцию,
     * метод вычисления которой сначала вызывает метод вычисления этой логической функции, потом вызывает метод
     * вычисления переданной в этот метод логической функции и, если результаты их вычисления неравны, возвращает
     * истинный результат вычисления, в противном случае возвращает ложный результат вычисления.
     *
     * @param condition логическая функция.
     *
     * @return Новая логическая функция, метод вычисления которой сначала вызывает метод вычисления этой логической
     * функции, потом вызывает метод вычисления переданной в этот метод логической функции и, если результаты их
     ...
     * истинный, возвращает истинный результат вычисления, в противном случае вызывает метод вычисления переданной в
     * этот метод логической функции, а после возвращает результат её вычисления.
     *
     * @param condition логическая функция.
     *
     * @return Новая логическая функция, метод вычисления которой вызывает метод вычисления этой логической функции и,
     * если результат её вычисления истинный, возвращает истинный результат вычисления, в противном случае вызывает
     * метод вычисления переданной в этот метод логической функции, а после возвращает результат её вычисления.
     *
     * @throws NullException исключение валидации нулевого объекта (переданной логической функции).
     * @since 1.0.0
     */
     ...
     * логическую функцию, метод вычисления которой вызывает метод вычисления этой логической функции и, если результат
     * её вычисления истинный, вызывает функцию обратного вызова, а после либо в противном случае возвращает результат
     ...
     * шаблонным сообщением), в противном случае инициализирует и возвращает логическую функцию, метод вычисления
     * которой вызывает метод вычисления этой логической функции и, если её результат вычисления истинный,
     * инициализирует и бросает исключение вычисления функции (этой логической функции) с переопределённой причиной
     * (полученным исключением либо ошибкой от переданного в этот метод поставщика), в противном случае возвращает
     * ложный результат вычисления.
     *
     ...
     * функции) с переопределённой причиной (полученным исключением либо ошибкой от переданного в этот метод
     ...
     * переданной функции обратного вызова шаблонным сообщением), в противном случае инициализирует и возвращает
     * логическую функцию, метод вычисления которой вызывает метод вычисления этой логической функции и, если результат
     * её вычисления ложный, вызывает функцию обратного вызова, а после либо в противном случае возвращает результат
     * вычисления этой логической функции.
     *
     * @param callback функция обратного вызова.
     *
     ...
     * объекта (переданного поставщика) с переопределённым сообщением (отформатированным именем переданного поставщика
     * шаблонным сообщением), в противном случае инициализирует и возвращает логическую функцию, метод вычисления
     * которой вызывает метод вычисления этой логической функции и, если её результат вычисления ложный, инициализирует
     * и бросает исключение вычисления функции (этой логической функции) с переопределённой причиной (полученным
     * исключением либо ошибкой от переданного в этот метод поставщика), в противном случае возвращает истинный
     * результат вычисления.
     *
     * @param supplier поставщик исключения либо ошибки.
     *
     * @return Новая логическая функция, метод вычисления которой вызывает метод вычисления этой логической функции и,
     * если её результат вычисления ложный, инициализирует и бросает исключение вычисления функции (этой логической
     * функции) с переопределённой причиной (полученным исключением либо ошибкой от переданного в этот метод
     * поставщика), в противном случае возвращает истинный результат вычисления.
     *
     * @throws NullException исключение валидации нулевого объекта (переданного поставщика).
     * @since 4.0.0
     */
