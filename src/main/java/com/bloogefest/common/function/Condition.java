...
public interface Condition {
    ...
    /**
     * Проверяет обратный вызов и, если он нулевой, инициализирует исключение валидации нулевого обратного вызова с
     * переопределённым сообщением, отформатированным именем обратного вызова шаблонным сообщением, в противном случае
     * инициализирует логическое выражение, метод вычисления которого проверяет результат вычисления этого логического
     * выражения и, если он истинный, вызывает функцию обратного вызова, а после возвращает результат вычисления этого
     * логического выражения.
     *
     ...
     * инициализирует логическое выражение, метод вычисления которого проверяет результат вычисления этого логического
     * выражения и, если он истинный, инициализирует исключение вычисления функции с переопределённой причиной,
     * полученным исключением либо ошибкой от поставщика, и бросает его, в противном случае возвращает ложный результат
     * вычисления результата этого логического выражения.
     *
     * @param supplier поставщик исключения либо ошибки.
     *
     * @return Логическое выражение с результатом вычисления этого логического выражения, при истинности которого
     * инициализируется исключение вычисления функции с переопределённой причиной, полученным исключением либо ошибкой
     * от поставщика, и бросается.
     *
     ...
     * переопределённым сообщением, отформатированным именем обратного вызова шаблонным сообщением, в противном случае
     * инициализирует логическое выражение, метод вычисления которого проверяет результат вычисления этого логического
     * выражения и, если он ложный, вызывает функцию обратного вызова, а после возвращает результат вычисления этого
     * логического выражения.
     *
     * @param callback обратный вызов.
     *
    ...
    /**
     * Проверяет поставщик и, если он ненулевой, инициализирует логическое выражение, метод вычисления которого
     * проверяет результат вычисления этого логического выражения и, если он ложный, инициализирует исключение
     * вычисления функции с переопределённой причиной, полученным исключением либо ошибкой от поставщика, и бросает его,
     * в противном случае возвращает истинный результат этого логического выражения.
     *
     * @param supplier поставщик исключения либо ошибки.
     *
     * @return Логическое выражение с результатом вычисления этого логического выражения, при ложности которого
