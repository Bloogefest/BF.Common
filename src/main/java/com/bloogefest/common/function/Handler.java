...
public interface Handler<T> {
    ...
    /**
     * Проверяет обработчик и обрабатываемый объект и, если те ненулевые, инициализирует другой обработчик, метод
     * обработки которого всегда вызывает метод обработки переданного в этот метод обработчика, используя переданный в
     * этот метод обрабатываемый объект, в противном случае инициализирует и бросает исключение валидации нулевого
     * объекта (обработчика либо обрабатываемого объекта) с переопределённым сообщением (отформатированным именем
     * обработчика либо обрабатываемого объекта шаблонным сообщением).
     *
     * @param handler обработчик.
     * @param object обрабатываемый объект.
     *
     * @return Новый обработчик, метод обработки которого всегда вызывает метод обработки переданного в этот метод
     * обработчика, используя переданный в этот метод обрабатываемый объект.
     *
     * @throws NullException исключение валидации нулевого объекта (обработчика либо обрабатываемого объекта).
     * @since 3.0.0
     */
     ...
     * исключение валидации нулевого объекта (обработчика).
     *
     * @param handler обработчик.
     *
