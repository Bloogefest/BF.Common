...
public interface Handler<T> {
    ...
    /**
     * Проверяет переданный обработчик и, если тот ненулевой, возвращает его, в противном случае инициализирует и
     * бросает исключение валидации нулевого объекта (переданного обработчика) с переопределённым сообщением
     * (отформатированным именем переданного обработчика шаблонным сообщением).
     *
     * @param handler обработчик объекта.
     *
     ...
     * инициализации обработчика, метод обработки которого ничего не делает, а после возвращает результат его
     * инициализации.
     *
     * @param handler обработчик объекта.
     *
     ...
     * вызывает метод обработки переданного в этот метод обработчика, также используя передаваемый в него обрабатываемый
     * объект.
     *
     * @param handler обработчик объекта.
     *
     ...
     * также используя передаваемый в него обрабатываемый объект.
     *
     * @throws NullException исключение валидации нулевого объекта (переданного обработчика).
     * @since 3.0.0
     */
