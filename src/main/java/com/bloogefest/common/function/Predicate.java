...
public interface Predicate<T> {
    ...
    /**
     * Инициализирует и возвращает предикат, метод оценивания которого возвращает переданный в этот метод результат
     * оценивания.
     *
     * @param result результат оценивания.
     *
     * @return Новый предикат, метод оценивания которого возвращает переданный в этот метод результат оценивания.
     *
     * @since 1.0.0
     */
    ...
    /**
     * Проверяет переданный предикат и, если тот нулевой, инициализирует и бросает исключение валидации нулевого объекта
     * (переданного предиката) с переопределённым сообщением (отформатированным именем переданного предиката шаблонным
     ...
     *
     * @throws NullException исключение валидации нулевого объекта (переданного предиката).
     * @apiNote Этот метод можно использовать для приведения лямбда-выражений к типу предиката объекта.
     * @since 1.0.0
     */
    ...
    /**
     * Инициализирует и возвращает предикат, метод оценивания которого возвращает инвертированный результат оценивания этого
     * предиката.
     * Инициализирует и возвращает предикат, метод оценивания которого вызывает метод оценивания этого
     * предиката, а после инвертирует и возвращает его результат оценивания.
     *
