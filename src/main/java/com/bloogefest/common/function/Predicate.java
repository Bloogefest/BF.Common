...
public interface Predicate<T> {
    ...
    /**
     * Проверяет обратный вызов и, если тот нулевой, инициализирует и бросает исключение валидации нулевого обратного
     * вызова с переопределённым сообщением (отформатированным именем обратного вызова шаблонным сообщением), в
     * противном случае инициализирует предикат, метод оценивания которого проверяет результат оценивания этого
     * предиката и, если тот истинный, вызывает функцию обратного вызова, а после возвращает результат оценивания этого
     * предиката.
     *
    ...
    /**
     * Проверяет обработчик и, если тот нулевой, инициализирует и бросает исключение валидации нулевого обработчика с
     * переопределённым сообщением (отформатированным именем обработчика шаблонным сообщением), в противном случае
     * инициализирует предикат, метод оценивания которого проверяет результат оценивания этого предиката и, если тот
     * истинный, вызывает функцию обработки объекта с оцениваемым объектом этого предиката, а после возвращает результат
     ...
     * объекта с оцениваемым объектом этого предиката.
     *
    ...
    /**
     * Проверяет поставщик и, если тот нулевой, инициализирует и бросает исключение валидации нулевого поставщика с
     * переопределённым сообщением (отформатированным именем поставщика шаблонным сообщением), в противном случае
     * инициализирует предикат, метод оценивания которого проверяет результат оценивания этого предиката и, если тот
     * истинный, инициализирует и бросает исключение оценивания объекта с переопределённой причиной (полученным
     * исключением либо ошибкой от поставщика), в противном случае возвращает ложный результат оценивания этого
     * предиката.
     *
     ...
     * исключение оценивания объекта с переопределённой причиной (полученным исключением либо ошибкой от поставщика).
     *
     ...
     * вызова с переопределённым сообщением (отформатированным именем обратного вызова шаблонным сообщением), в
     * противном случае инициализирует предикат, метод оценивания которого проверяет результат оценивания этого
     * предиката и, если тот ложный, вызывает функцию обратного вызова, а после возвращает результат оценивания этого
     * предиката.
     *
     * @param callback обратный вызов.
     *
     ...
     * противном случае инициализирует предикат, метод оценивания которого проверяет результат оценивания этого
     * предиката и, если тот ложный, вызывает функцию обработки объекта с оцениваемым объектом этого предиката, а после
     * возвращает результат оценивания этого предиката.
     *
     * @param handler обработчик объекта.
     *
     ...
     * противном случае инициализирует предикат, метод оценивания которого проверяет результат оценивания этого
     * предиката и, если тот ложный, инициализирует и бросает исключение оценивания объекта с переопределённой причиной
     * (полученным исключением либо ошибкой от поставщика), в противном случае возвращает истинный результат оценивания
     * этого предиката.
     *
     * @param supplier поставщик исключения либо ошибки.
     *
