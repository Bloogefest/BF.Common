...
public interface Predicate<T> {
    ...
    /**
     * Проверяет переданную предикативную функцию и, если та нулевая, генерирует исключение валидации нулевого объекта
     * (переданной предикативной функции) с переопределённым сообщением (отформатированным именем переданной
     * предикативной функции шаблонным сообщением), в противном случае возвращает её.
     *
     ...
     * Проверяет переданную предикативную функцию и, если та нулевая, генерирует исключение валидации нулевого объекта
     * (переданной предикативной функции) с переопределённым сообщением (отформатированным именем переданной
     * предикативной функции шаблонным сообщением), в противном случае инициализирует другую предикативную функцию,
     * метод оценивания которой вызывает метод оценивания этой предикативной функции, используя передаваемый в него
     * оцениваемый объект и, если её результат оценивания истинный, вызывает метод оценивания переданной в этот метод
     * предикативной функции, вновь используя передаваемый в него оцениваемый объект, а после возвращает её результат
     * оценивания, в противном случае возвращает ложный результат оценивания.
     *
     * @param predicate предикативная функция.
     *
     ...
     * (переданной предикативной функции) с переопределённым сообщением (отформатированным именем переданной
     * предикативной функции шаблонным сообщением), в противном случае инициализирует другую предикативную функцию,
     * метод оценивания которой сначала вызывает метод оценивания этой предикативной функции, используя передаваемый в
     * него оцениваемый объект, потом вызывает метод оценивания переданной в этот метод предикативной функции, вновь
     * используя передаваемый в него оцениваемый объект, и, если их результаты оценивания неравны, возвращает истинный
     ...
     * предикативной функции шаблонным сообщением), в противном случае инициализирует другую предикативную функцию,
     * метод оценивания которой вызывает метод оценивания этой предикативной функции, используя передаваемый в него
     * оцениваемый объект, и, если её результат оценивания истинный, возвращает истинный результат оценивания, в
     ...
     * функции обратного вызова шаблонным сообщением), в противном случае инициализирует и возвращает предикативную
     * функцию, метод оценивания которой вызывает метод оценивания этой предикативной функции и, если её результат
     * оценивания истинный, вызывает функцию обратного вызова, а после либо в противном случае возвращает результат
     * оценивания этой предикативной функции.
     *
     ...
     * в противном случае инициализирует и возвращает предикативную функцию, метод оценивания которой вызывает метод
     * оценивания этой предикативной функции и, если её результат оценивания истинный, генерирует исключение оценивания
     * объекта (переданного в этот метод оцениваемого объекта) с переопределённой причиной (полученным исключением либо
     * ошибкой от переданного в этот метод поставщика), в противном случае возвращает ложный результат оценивания.
     *
     * @param supplier поставщик исключения либо ошибки.
     *
     * @return Новая предикативная функция, метод оценивания которой вызывает метод оценивания этой предикативной
     * функции и, если её результат оценивания истинный, генерирует исключение оценивания объекта (переданного в этот
     * метод оцениваемого объекта) с переопределённой причиной (полученным исключением либо ошибкой от переданного в
     * этот метод поставщика), в противном случае возвращает ложный результат оценивания.
     *
     ...
     * функции обратного вызова шаблонным сообщением), в противном случае инициализирует и возвращает предикативную
     * функцию, метод оценивания которой вызывает метод оценивания этой предикативной функции и, если её результат
     * оценивания ложный, вызывает функцию обратного вызова, а после либо в противном случае возвращает результат
     * оценивания этой предикативной функции.
     *
     * @param callback функция обратного вызова.
     *
     ...
     * поставщика) с переопределённым сообщением (отформатированным именем переданного поставщика шаблонным сообщением),
     * в противном случае инициализирует и возвращает предикативную функцию, метод оценивания которой вызывает метод
     * оценивания этой предикативной функции и, если её результат оценивания ложный, генерирует исключение оценивания
     * объекта (переданного в этот метод оцениваемого объекта) с переопределённой причиной (полученным исключением либо
     * ошибкой от переданного в этот метод поставщика), в противном случае возвращает ложный результат оценивания.
     *
     * @param supplier поставщик исключения либо ошибки.
     *
     * @return Новая предикативная функция, метод оценивания которой вызывает метод оценивания этой предикативной
     * функции и, если её результат оценивания ложный, генерирует исключение оценивания объекта (переданного в этот
     * метод оцениваемого объекта) с переопределённой причиной (полученным исключением либо ошибкой от переданного в
     * этот метод поставщика), в противном случае возвращает ложный результат оценивания.
     *
     * @throws NullException исключение валидации нулевого объекта (переданного поставщика).
