...
/**
 * Данный класс представляет собой строгое исключение и применяется в некритических ситуациях, требующих немедленного
 * реагирования. Для этого он расширяет класс исключения, а также объявляет и реализует наиболее полезные конструкторы и
 * статические поля.
 *
 * @apiNote После возникновения и обработки строгого исключения обычно можно продолжить работу. Например, когда
 * происходит неожиданное закрытие файлового потока.
 * @since 3.0.0-RC1
 */
public class StrictException extends Exception {

    /**
     * Данное неизменяемое статическое поле представляет собой нулевой объект класса строки и содержит сообщение
     * строгого исключения по умолчанию.
     *
     * @since 4.0.0-RC3
     */
    ...
    /**
     * Данный конструктор создаёт строгое исключение на основе {@linkplain #DEFAULT_MESSAGE сообщения},
    ...
    /**
     * Данный конструктор создаёт строгое исключение на основе переданного сообщения, а также
     * {@linkplain #DEFAULT_CAUSE причины}, {@linkplain #DEFAULT_SUPPRESSION параметров подавления} и
     * {@linkplain #DEFAULT_WRITABLE трассировки стека по умолчанию}.
     *
    ...
    /**
     * Данный конструктор создаёт строгое исключение на основе переданной причины, а также
    ...
    /**
     * Данный конструктор создаёт строгое исключение на основе переданного сообщения и причины, а также
    ...
    /**
     * Данный конструктор создаёт строгое исключение на основе переданных параметров подавления и трассировки стека, а
     * также {@linkplain #DEFAULT_MESSAGE сообщения} и {@linkplain #DEFAULT_CAUSE причины по умолчанию}.
     *
    ...
    /**
     * Данный конструктор создаёт строгое исключение на основе переданного сообщения, параметров подавления и
     * трассировки стека, а также {@linkplain #DEFAULT_CAUSE причины по умолчанию}.
     *
     * @param message сообщение.
     * @param suppression параметр подавления.
     * @param writable параметр трассировки стека.
    ...
    /**
     * Данный конструктор создаёт строгое исключение на основе переданной причины, параметров подавления и трассировки
    ...
    /**
     * Данный конструктор создаёт строгое исключение на основе переданного сообщения, причины, параметров подавления и
     * трассировки стека.
     *
     * @param message сообщение.
     * @param cause причина.
